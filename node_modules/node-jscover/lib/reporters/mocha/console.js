(function (mocha) {
    ...
    var JscoverReporter = function (runner) {
        ...
        runner.on('end', function () {
            if (g._$jscoverage) {
                printCoverageInfo();
                try {
                    var statPage = g.opener || g.parent;
                    if (statPage && statPage.document.getElementById('summaryTab')) {
                        statPage.document.getElementById('summaryTab').click();
                        if(g.opener) {
                            g.close();
                        }
                    }
    ...
    Noop.prototype = OriginalReporter.prototype;

    JscoverReporter.prototype = new Noop();

    if (window.mochaPhantomJS) {
        exports = JscoverReporter;
    } else {
        mocha.reporter(JscoverReporter);
    }
})(mocha);